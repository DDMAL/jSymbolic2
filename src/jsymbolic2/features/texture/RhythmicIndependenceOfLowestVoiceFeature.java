 ...
 * lowest average pitch has a new note onset and this is the only new note onset in the slice, divided by the 
 * total number of note onset slices where the MIDI track/channel voice with the lowest average pitch has a 
 * new note onset. Set to 0 if there are only zero or one voices containing pitched notes. MIDI Channel 10 
 * notes are ignored.
 *
 * @author radamian
 */
public class RhythmicIndependenceOfLowestVoiceFeature
	...
	throws Exception
	{
		...
		if (sequence_info != null)
		{
			...
			else
			{
				...
				// The number of note onset slices where the lowest voice has a new onset
				int number_of_slices_with_new_onset_in_lowest_voice = 0;
				// The number of note onset slices where only the lowest voice has a new onset
				int number_of_slices_with_independent_lowest_voice = 0;
				
				// Whether the lowest voice has a new onset in the current slice
				boolean onset_in_lowest_voice;
					...
					for (int n_track = 0; n_track < note_onset_slices_by_track_and_channel.length; n_track++)
						for (int chan = 0; chan < note_onset_slices_by_track_and_channel[n_track].length; chan++)
							if (!note_onset_slices_by_track_and_channel[n_track][chan].get(i).isEmpty())
							{
								...
								{
									onset_in_lowest_voice = true;
									number_of_slices_with_new_onset_in_lowest_voice++;
								}
							}
					
					// Increment number_of_slices_with_independent_lowest_voice if the only note onset in this 
					// slice belongs to the lowest voice
					if (onset_in_lowest_voice && voices_with_new_onsets == 1)
						number_of_slices_with_independent_lowest_voice++;
				}
				
				// Set value
				value = (double) number_of_slices_with_independent_lowest_voice / number_of_slices_with_new_onset_in_lowest_voice;
			}
		}
		else value = -1.0;
