 ...
 * highest average pitch that fall outside the range of any other pitched voice, divided by the total number 
 * of notes in the voice with the highest average pitch. Set to 0 if there are only 0 or 1 voices containing 
 * pitched notes.
 *
 * @author radamian
 */
public class RelativeRangeIsolationOfHighestVoiceFeature
		extends MIDIFeatureExtractor
{
	 ...
	 */
	public RelativeRangeIsolationOfHighestVoiceFeature()
	{
		String name = "Relative Range Isolation of Highest Voice";
		String code = "T-16";
		String description = "Number of pitched notes in the MIDI track/channel voice with the highest average pitch that fall outside the range of any other pitched voice, divided by the total number of notes in the voice with the highest average pitch. Set to 0 if there are only 0 or 1 voices containing pitched notes.";
	...
	throws Exception
	{
		...
		if (sequence_info != null)
		{
			...
			else
			{
				...
				// MIDI track/channel pairing with the highest average pitch
				int lowest = 128;
				int highest = -1;
				for (int n_track = 0; n_track < sequence_info.track_and_channel_statistics.length; n_track++)
					for (int chan = 0; chan < sequence_info.track_and_channel_statistics[n_track].length; chan++)
					{
						if ((sequence_info.track_and_channel_statistics[n_track][chan][0] != 0 && chan != 10 - 1) &&
							(n_track != track_highest_voice && chan != channel_highest_voice))
						{
				...
				// All notes in the highest voice
				List<NoteInfo> notes_in_highest_voice = sequence_info.all_notes.getNotesOnTrackAndChannel(track_highest_voice, channel_highest_voice);
				
				// Count the notes in the highest voice that fall outside the range of any other voice
				int outside_of_range_count = 0;
				for (int i = 0; i < notes_in_highest_voice.size(); i++)
					if (notes_in_highest_voice.get(i).getPitch() < lowest || 
						notes_in_highest_voice.get(i).getPitch() > highest)
						outside_of_range_count++;
				
				// Set value
				value = (double) outside_of_range_count / notes_in_highest_voice.size();
			}
		}
		else value = -1.0;
