 ...
 * semitones are wrapped (e.g. an octave (12 semitones) is added to the bin for unisons (0 semitones)). Rests
 * and periods where only a single note is sounding are ignored when this histogram is calculated. The result
 * is a histogram that indicates which vertical intervals are present, and how significant these vertical
 * intervals are relative to one another, with a weighting based on both MIDI velocity and the aggregated
 * durations with which each interval is held throughout the piece. This histogram is first normalized, then
 * bin magnitudes under .001 are filtered out and set to 0 to reduce noise. Finally, the histogram is
 * re-normalized.
 *
