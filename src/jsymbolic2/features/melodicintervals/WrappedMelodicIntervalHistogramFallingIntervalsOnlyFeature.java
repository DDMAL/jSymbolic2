 ...
 * found separately for each track and channel before being combined in this histogram. It is also assumed 
 * that there is only one melody at a time per MIDI channel (if multiple notes occur simultaneously on the 
 * same MIDI tick on the same MIDI track and channel, then all notes but the first note on that tick are 
 * ignored). Other than this, all notes on the same track and the same channel are treated as if they are 
 * part of a single melody. It is also assumed that melodies do not cross MIDI tracks or channels (i.e. that 
 * they are each separately contained in their own track and channel). Only pitched notes are considered, so 
 * all notes on the unpitched MIDI Channel 10 are ignored.
 *
 * @author radamian
 */
