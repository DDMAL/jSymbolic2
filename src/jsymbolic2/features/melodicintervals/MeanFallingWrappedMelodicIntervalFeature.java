 ...
 * falling wrapped melodic intervals in the piece.
 *
 * @author radamian
 */
		...
		extends MIDIFeatureExtractor
{
	...
	throws Exception
	{
		...
		{
			// Get wrapped melodic interval histogram
			double[] wrapped_melodic_interval_histogram_falling_intervals_only = other_feature_values[0];
			
			value = 0.0;
			for (int bin = 0; bin < wrapped_melodic_interval_histogram_falling_intervals_only.length; bin++)
				value += (double) bin * wrapped_melodic_interval_histogram_falling_intervals_only[bin];
		}
		else value = -1.0;
