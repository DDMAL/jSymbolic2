 ...
 * MIDI track and channel with the highest average pitch. Rising and falling intervals are treated as 
 * equivalent.
 *
 * @author radamian
 */
		...
		extends MIDIFeatureExtractor
{
	...
	public NumberOfDistinctMelodicIntervalsInHighestLineFeature()
	{
		...
		String code = "M-94";
		String description = "Number of distinct melodic intervals that occur at least once in the MIDI track and channel with the highest average pitch. Rising and falling intervals are treated as equivalent.";
	...
	throws Exception
	{
		...
		if (sequence_info != null)
		{
			...
			for (int i = 0; i < sequence_info.melodic_intervals_by_track_and_channel.get(track_with_highest_average_pitch)[channel_with_highest_average_pitch].size(); i++)
				if (!unique_melodic_intervals.contains(Math.abs(sequence_info.melodic_intervals_by_track_and_channel.get(track_with_highest_average_pitch)[channel_with_highest_average_pitch].get(i))))
					unique_melodic_intervals.add(Math.abs(sequence_info.melodic_intervals_by_track_and_channel.get(track_with_highest_average_pitch)[channel_with_highest_average_pitch].get(i)));
