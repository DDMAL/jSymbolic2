 ...
 * above. Entries for melodic intervals over an octave are amalgamated (e.g. 2nds and 9ths are combined). Each
 ...
 * to the fraction of melodic intervals in the piece that are of the kind associated with the bin (this
 * histogram is normalized). Rising and falling intervals are treated as identical. Melodies are assumed to be
 * contained within individual MIDI tracks and channels, so melodic intervals are found separately for each
 * track and channel before being combined in this histogram. If a given note onset slice on a given MIDI
 * track and channel contains multiple pitches, then only the highest pitched note in the slice is counted for
 ...
 * melody, even if lower pitches in the same track and channel change. It is also assumed that melodies do not
 * cross MIDI tracks or channels (i.e. that they are each separately contained in their own track and
 * channel). Only pitched notes are considered, so all notes on the unpitched MIDI Channel 10 are ignored.
 *
 * @author radamian
 */
