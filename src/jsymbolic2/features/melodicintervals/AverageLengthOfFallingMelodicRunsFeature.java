 ...
 */
public class AverageLengthOfFallingMelodicRunsFeature
		extends MIDIFeatureExtractor
{
	 ...
	 */
	public AverageLengthOfFallingMelodicRunsFeature()
	{
		String name = "Average Length of Falling Melodic Runs";
		String code = "M-74";
		String description = "Average number of consecutive melodically falling notes that occur before a melodic change of direction, including the two end notes. Melodic unisons are not considered to break a run. Similar assumptions are made in the calculation of this feature as for the Melodic Interval Histogram. Set to 0 if no such runs are found.";
			...
			for (int track = 0; track < sequence_info.melodic_intervals_by_track_and_channel.size(); track++)
			{
				...
				for (int chan = 0; chan < melodic_intervals_by_channel.length; chan++)
				{
					if (chan != (10 - 1))
					{
						...
						// Find the length of each falling melodic run
						for (int i = 0; i < intervals.length; i++)
						{
