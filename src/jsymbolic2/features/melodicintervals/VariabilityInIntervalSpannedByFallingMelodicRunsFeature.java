 ...
 * the highest note and the lowest note in runs of consecutive melodically falling notes. Melodic unisons are 
 * not considered to break a run. Similar assumptions are made in the calculation of this feature as for the 
 * Melodic Interval Histogram. Set to 0 if no melodic such runs are found.
 *
 * @author radamian and Cory McKay
 */
public class VariabilityInIntervalSpannedByFallingMelodicRunsFeature
		extends MIDIFeatureExtractor
{
	 ...
	 */
	public VariabilityInIntervalSpannedByFallingMelodicRunsFeature()
	{
		String name = "Variability in Interval Spanned by Falling Melodic Runs";
		String code = "M-80";
						...
						for (int i = 0; i < intervals.length; i++)
						{
							...
							if (direction == -1)
							{
								if (intervals[i] > 0)
								{
									...
									interval_spanned_by_run += intervals[i];
							}
							...
							if (i == intervals.length - 1 && interval_spanned_by_run < 0)
								intervals_spanned_by_falling_runs.add(Math.abs(interval_spanned_by_run));
						}
					}
				}
